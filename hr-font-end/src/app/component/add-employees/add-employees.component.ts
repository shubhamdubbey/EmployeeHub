...
export class AddEmployeesComponent {
  ...
  constructor(private humanResourceService: HumanResourceService) {
    this.humanResourceService.getGrades().subscribe((data) => {
      this.grade = data;
    });
  }
  ...
  save() {
    this.humanResourceService.registerEmployee(this.users).subscribe(
      (data) => {
        this.submitted = true;
        this.check = true;
      },
  ...
  onSubmit() {
    ...
    this.users.managerId = +this.addEmployeeForm.get('homeManagerId').value;
    this.save();
  }
}
